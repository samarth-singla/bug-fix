...
async def get_analytics():
    ...
    users = []      # removed hard coded "A1","B2","C3" that can corrupt user data
    ...
    item_name_lengths = np.array([len(item["name"]) for item in items]) if items else np.array([])  # changed names to name
    user_username_lengths = np.array([len(user["username"]) for user in users]) if users else np.array([])      # changed usernames to username
    ...
    return JSONResponse({
        "stats": stats,
        "chart": image_base64       # added "chart": image_base64
    })